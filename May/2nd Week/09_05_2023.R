...
calculate_percentage_change <- function(mcsa) {
  ...
  if (is.na(cost_before) | is.na(cost_after)) {
    ...
    prc_chng <- ((cost_after - cost_before) / cost_before) * 100
    return (prc_chng)
  }
}
...
to_plot_counties <- left_join(x = counties_sf, y = by_counties, by = c("FIPS"="county_fips_code"))
...
ggplot(to_plot_states) +
  geom_sf(aes(fill = mfccsa)) +
  scale_fill_gradient2(low = "green", mid = "white", high = "red", midpoint = 0, na.value = "grey") +
  labs(title = "Average Change Cost by State") +
  theme_void() +
  coord_sf(xlim = c(-140, -70), ylim = c(10, 60)) +
  theme(plot.title = element_text(hjust = 0.5, size = 20))

ggplot(to_plot_counties) +
  geom_sf(aes(fill = mfccsa)) +
  scale_fill_gradient(low = "#b1b1ff", high = "#0000ff", na.value = "white") +
